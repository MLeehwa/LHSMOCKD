...
type TwoColumnHandsontableProps = {
	onSave: (data: Array<{ date: string; barcode: string }>) => Promise<void>;
	initialData?: Array<{ date: string; barcode: string }>;
};

export function TwoColumnHandsontable({ onSave, initialData = [] }: TwoColumnHandsontableProps) {
	...
	useEffect(() => {
		...
		const data = initialData.length > 0 
			? initialData.map(item => [item.date, item.barcode])
			: [];

		const hot = new Handsontable(containerRef.current, {
			data,
			colHeaders: ["Date (YYYY-MM-DD)", "Barcode"],
			rowHeaders: true,
			width: "100%",
			height: 400,
			minSpareRows: 5,
	...
	const handleSave = async () => {
		...
		try {
			...
			const allRows = rawData
				.filter(row => row[0] && row[1]) // 날짜와 바코드 둘 다 있는 행만
			...
			const uniqueRows: Array<{ date: string; barcode: string }> = [];
	...
	return (
		<div className="rounded border border-purple-300 bg-purple-50 p-3">
			...
			<div className="text-xs text-gray-600 mt-2">
				Enter date (YYYY-MM-DD) and barcode, or paste from Excel (Ctrl+V). Supports unlimited rows.
			</div>
		</div>
	);
}
